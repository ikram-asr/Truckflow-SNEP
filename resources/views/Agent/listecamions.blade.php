...
@section('nav-item')
<form action="{{ route('pdfag') }}" method="GET">
...
<div class="table-responsive "  style="       
    ...
    margin: 10px;      background: #F6FBF9;
            ...
            box-shadow: 0px 4px 10px 1px rgba(0,0,0,0.1);" >
                        <form action="{{ route('filtre') }}" method="GET" class="mb-3">
    @csrf
    <div class="row  d-flex justify-content-right align-items-center ">
        ...
        <div class="col-md-1">
            <a href="{{ route('listecamions') }}" class="btn btn-secondary" style="color: #fff; background: #f44336; border: 0px; cursor: pointer;">Effacer</a>
        </div>
    </div>
</form>
  <table class="table table-hover table-striped table align-middle table-responsive-sm table-responsive-md
  table-responsive-lg">
  <table id="cami" class="table table-hover table-striped table align-middle table-responsive-sm table-responsive-md
  table-responsive-lg">
  <thead>
    <tr>
...
@if (session('pdfdata'))
    <script>
        window.onload = function() {
            ...
            window.location.href = "{{ route('fichecam') }}";
        };
    </script>
@endif
</div>
